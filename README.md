...
│
├── app/
│   ├── streamlit_app.py          # Module 1 — Scheduling UI + Customer CSAT
│   ├── module2_training.py       # Module 2 — Training & Compliance Dashboard
│   └── module3_hiring.py         # Module 3 — Hiring & Morale Dashboard
│
...
| 3 | `qsr_customer_survey.csv` | 1,830 | 16 | 1, 3 | ~5 surveys/day for 2024. Ratings for speed, friendliness, accuracy, cleanliness, product availability. Includes CSAT score and NPS flag. Surfaced in Module 1 scheduler per ISO week. |
| 4 | `qsr_employee_survey.csv` | 780 | 17 | 3 | Weekly morale survey for all 15 employees across 52 weeks. Tracks the 6 morale factors, morale_index, and at_risk_flag for Trigger T4. Powers Module 3 morale and satisfaction report tabs. |
...
### Customer Satisfaction Panel (⭐ CSAT)

Every time a week is selected, a collapsible **Customer Satisfaction** panel appears below the week header — before you even generate the schedule. It surfaces historical 2024 survey data matched by ISO week number:

- **KPIs:** avg CSAT score, speed of service, order accuracy, likelihood to return, NPS score
- **NPS breakdown:** Promoter / Passive / Detractor counts with health indicator (green ≥ 20, yellow ≥ 0, red < 0)
- **CSAT by shift period:** morning vs afternoon vs evening
- **5-factor bar chart:** speed, friendliness, accuracy, cleanliness, availability
- **Peak vs off-peak comparison:** surfaces whether CSAT drops during the 12–13 and 18–19 peak hours — a direct staffing-quality signal
- **Hourly CSAT trend line**

This closes the scheduling feedback loop: the GM can see *"last time we ran this week, CSAT at peak was 3.81 vs 3.90 off-peak"* before deciding how to staff the upcoming schedule.
...
### Module 2 Streamlit Dashboard (`app/module2_training.py`)

Four-tab interactive dashboard covering the full training and compliance picture:

| Tab | What It Shows |
|---|---|
| 🔴 Compliance Certs | Expired/expiring-soon alert cards, full colour-coded cert table, per-certification coverage progress bars (e.g. WHMIS at 40% compliant), employee compliance scorecard |
| 📊 Station Training | Proficiency heatmap (0–4 scale, grey → dark green) for all 15 employees × 8 stations, station coverage summary, filterable training records table, avg score bar chart by station |
| 🌱 Train-the-Trainer | 5-stage pipeline badges per station derived live from training records, succession risk warnings (0 or 1 trainer per station), overall pipeline funnel, certified trainer roster |
| 👤 Employee Profile | Select any employee → station proficiency tiles, all compliance certs with status, full training record history, assessment score trend chart |

**Sidebar filters:** employee, certification type, renewal status, station — all tabs respond simultaneously.
...
### Module 3 Streamlit Dashboard (`app/module3_hiring.py`)

Five-tab interactive dashboard covering hiring triggers, capacity gaps, and team morale:

| Tab | What It Shows |
|---|---|
| 🚨 Hiring Triggers | Priority action cards (red/yellow/green by severity), full trigger table, headcount needed by role bar chart, T1–T7 trigger legend |
| 📈 Demand & Capacity | 52-week demand vs capacity line chart (427 avg vs 311 cap = 116 hrs/wk structural gap), T6 peak-week table, per-employee capacity breakdown |
| 💚 Team Morale | At-risk employee alert cards, 52-week morale trend, 6-factor breakdown with weights, per-role morale comparison, 15×52 employee-week heatmap |
| 👤 Employee Morale Profile | 52-week morale + retention trend per employee, factor scores vs team average, full detail table with at-risk week highlighting (defaults to Emma Leblanc) |
| 📋 Satisfaction Report | Executive annual summary: key findings, quarterly breakdown, factor rankings with weighted contribution, retention risk by role, 5 prioritised recommendations, CSV + TXT export |
...
Each module is a standalone Streamlit app. Run them in separate terminals:

```bash
# Module 1 — Smart Schedule Optimizer + Customer CSAT
streamlit run app/streamlit_app.py

# Module 2 — Training & Compliance Dashboard
streamlit run app/module2_training.py

# Module 3 — Hiring Needs & Morale Dashboard
streamlit run app/module3_hiring.py
```

Each app opens at `http://localhost:8501` (or the next available port if others are running).
...
| Customer survey data (1,830 rows) | ✅ Complete |
| Employee morale survey data (780 rows) | ✅ Complete |
| Streamlit app — calendar UI, per-day overrides, event presets | ✅ Complete |
| Customer CSAT panel — per ISO week, NPS, peak vs off-peak | ✅ Complete |
...
| Compliance expiry alerts (Expired / Expiring Soon / Current) | ✅ Complete |
| Station proficiency heatmap (15 employees × 8 stations) | ✅ Complete |
| Train-the-Trainer pipeline tracker (5 stages, per station) | ✅ Complete |
| Employee training & compliance profile view | ✅ Complete |
...
| Demand vs capacity 52-week chart | ✅ Complete |
| Team morale dashboard + 15×52 heatmap | ✅ Complete |
| Employee morale profile (52-week trend, factor comparison) | ✅ Complete |
| Employee Satisfaction Report — annual executive summary | ✅ Complete |
| CSV + TXT report exports | ✅ Complete |
